...
simsignal_t DSMEPlatform::sentL2CCNFrame;
...
void DSMEPlatform::request_slot()
{
    ...
    */
    ...
    /* TODO */
    const char *destAddrs = getParentModule()->getParentModule()->getSubmodule("trafficgen")->par("destAddresses");

    std::regex rgx("host\\[([0-9]+)\\]");

    std::smatch matches;
    std::string s(destAddrs, destAddrs + strlen(destAddrs));
    std::regex_search(s, matches, rgx);

    EV_INFO << "DEST_ADDR->" << matches[1].str() << endl;

    std::string parent = "Net802154.host[" + matches[1].str() + "].wlan[0].mac";
    cModule *submod = getSimulation()->getModuleByPath(parent.c_str());
    DSMEPlatform *gw = check_and_cast<DSMEPlatform*>(submod);

    int channel = std::stoi(matches[1].str());
    EV_INFO << "CHANNEL: " << channel << endl;
    /*
    if (par("gtsTx")) {
    */
        EV_INFO << "Host[" << getParentModule()->getParentModule()->getIndex() << "]: GTS: SId: " << tx_superframe_id << " slotID: " << tx_slot_id << " Direction: TX" << endl;
        gw->allocateGTS(tx_superframe_id, tx_slot_id, channel, Direction::RX, this->mac_pib.macExtendedAddress.getShortAddress());
        //allocateGTS(tx_superframe_id, tx_slot_id, 0, Direction::TX, this->panDescriptorToSyncTo.coordAddress.getShortAddress());
        allocateGTS(tx_superframe_id, tx_slot_id, channel, Direction::TX, gw->mac_pib.macExtendedAddress.getShortAddress());
        recordScalar("txSuperframeID", tx_superframe_id);
        recordScalar("txSlotID", tx_slot_id);
        recordScalar("txChannel", 11 + channel);
    /*
    }
    */
...
void DSMEPlatform::handleDISASSOCIATION_confirm(mlme_sap::DISASSOCIATE_confirm_parameters& params) {
...
void DSMEPlatform::handleASSOCIATION_confirm(mlme_sap::ASSOCIATE_confirm_parameters& params) {
    ...
    if(params.status == AssociationStatus::SUCCESS) {
        ...
        if (!(strcmp(schedulingSelection, "STATIC") == 0)) {
            if (!par("isPANCoordinator") && !par("isCoordinator")) {
                request_slot();
...
void DSMEPlatform::handleStartOfCFP() {
...
void DSMEPlatform::handleCOMM_STATUS_indication(mlme_sap::COMM_STATUS_indication_parameters& params) {
...
GTS DSMEPlatform::getNextFreeGTS_CAP_CFP(uint16_t initialSuperframeID, uint8_t initialSlotID, const DSMESABSpecification* sabSpec, GTSType typeOfGTS) {
...
GTS DSMEPlatform::getNextFreeGTS(uint16_t initialSuperframeID, uint8_t initialSlotID, const DSMESABSpecification* sabSpec) {
    GTS gts(0,0,0);
...
void DSMEPlatform::findFreeSlots(DSMESABSpecification& requestSABSpec, DSMESABSpecification& replySABSpec, uint8_t numSlots, uint16_t preferredSuperframe,
...
void DSMEPlatform::handleDSME_GTS_indication(mlme_sap::DSME_GTS_indication_parameters& params) {
...
void DSMEPlatform::handleDSME_GTS_confirm(mlme_sap::DSME_GTS_confirm_parameters& params) {
...
void DSMEPlatform::initialize(int stage) {
    ...
    if(stage == INITSTAGE_LOCAL) {
        ...
        this->mlme_sap.getDISASSOCIATE().confirm(DELEGATE(&DSMEPlatform::handleDISASSOCIATION_confirm, *this));

        this->dsme->setStartOfCFPDelegate(DELEGATE(&DSMEPlatform::handleStartOfCFP, *this)); /* BAD cross-layer hack */
        ...
        this->mlme_sap.getCOMM_STATUS().indication(DELEGATE(&DSMEPlatform::handleCOMM_STATUS_indication, *this));
        ...
        if(!strcmp(schedulingSelection, "STATIC")) {
            ...
            for(auto &slot : slots) {
                allocateGTS(slot.superframeID, slot.slotID, slot.channelID, (Direction)slot.direction, slot.address);
            }
        }
...
DSMELoRaMessage* DSMEPlatform::getEmptyMessage() {
    messagesInUse++;
...
DSMELoRaMessage* DSMEPlatform::getLoadedMessage(inet::Packet* packet) {
    messagesInUse++;
...
void DSMEPlatform::signalNewMsg(DSMELoRaMessage* msg) {
#if 0
    LOG_INFO_PREFIX;
    LOG_INFO_PURE(msgId << " - ");
